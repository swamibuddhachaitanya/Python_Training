...
# Write to log_report.txt
with open(report_txt_path, 'w') as report_txt:
    report_txt.write("    IP                  DATE            PICS                URL\n")
    for ip, date, pics, url in zip(ip_list, date_list, pics_list, url_list):
        report_txt.write(f"{ip:<15} {date:<18} {pics:<15} {url}\n")

# Write to log_report.csv
with open(report_csv_path, 'w') as report_csv:
    report_csv.write("    IP                  DATE            PICS                URL\n")
    for ip, date, pics, url in zip(ip_list, date_list, pics_list, url_list):
        report_csv.write(f"{ip},   {date},   {pics},   {url}\n")
