"""
USer ORM for validatelogin endpoint
"""
...
def my_addnewuser_page():
    ...
    get_entry = myuserstable.query.get(entered_username)

    # if get_entry is not None:
    if get_entry:
        return "User already exist.<br><a href='/register'>Go Back</a>"
    else:
        new_entry = myuserstable(user_name=entered_username, user_password= entered_password_1, user_email=entered_email)
...
def my_validatelogin_page():
    ...
    # Task-2: Validate with db and return success or failure
    ############
    get_entry = myuserstable.query.filter_by(user_name=entered_username, user_password=entered_password).first()
    ...
    if get_entry:
        ...
        return flask.render_template("newlogreport.html", my_db_data={"my_log_db_result": my_log_db_result,
