...
# Write to log_report.txt using print function
with open(report_txt_path, 'w') as report_txt:
    print("    IP                  DATE            PICS                URL", file=report_txt)
    for ip, date, pics, url in zip(ip_list, date_list, pics_list, url_list):
        print(f"{ip:<15} {date:<18} {pics:<15} {url}", file=report_txt)

# Write to log_report.csv using print function
with open(report_csv_path, 'w') as report_csv:
    print("IP,DATE,PICS,URL", file=report_csv)
    for ip, date, pics, url in zip(ip_list, date_list, pics_list, url_list):
        print(f"{ip},   {date},   {pics},   {url}", file=report_csv)
